...
public class Drive : MonoBehaviour
{
    ...
    void CalculateAngle() //dot product video
    {
        Vector3 tankForward = transform.up;
        Vector3 fuelDirection = fuel.transform.position - transform.position;

        Debug.DrawRay(this.transform.position, tankForward * 10, Color.green, 4);
        Debug.DrawRay(this.transform.position, fuelDirection, Color.red, 4);
    ...
    Vector3 Cross(Vector3 v, Vector3 w) //cross product
    {
    ...
    float CalculateDistance() //distance video
    {
    ...
    void LateUpdate()
    {
        ...
        if(Input.GetKeyDown(KeyCode.T)) //auto pilot vid
        {
            autopilot = !autopilot;
        }
